...
//  $sql = "SELECT * FROM advertisement WHERE Supplier_Id = '".$suplierid."' ";
 $sql = "SELECT advertisement.Ad_Id,main_categories.main_cat_name,main_sub_categories.sub_catname,advertisement.Ad_Date AS AD_Date,advertisement.Adver_Status,supplier_detail.Name,districts.District_name AS DIS_Name,agri_center.Name AS AGRI_Name, advertisement_images.Image_Url, advertisement_images.Img_Auto_ID,advertisement.Unit_Price FROM `advertisement` LEFT JOIN main_categories ON main_categories.main_cat_id=advertisement.Cat_Id LEFT JOIN main_sub_categories ON main_sub_categories.sub_catid=advertisement.Sub_Cat_id LEFT JOIN supplier_detail ON supplier_detail.Supplier_ID=advertisement.Supplier_Id LEFT JOIN districts ON districts.dis_auto_id=supplier_detail.sup_district LEFT JOIN agri_center ON agri_center.Center_ID=supplier_detail.sup_govicenter LEFT JOIN advertisement_images ON advertisement_images.Ad_Id=advertisement.Ad_Id WHERE  main_categories.main_cat_id='".$maincatid."' AND main_sub_categories.sub_catid='".$subcatid."' AND advertisement.Adver_Status=1 GROUP BY advertisement.Ad_Id DESC";
